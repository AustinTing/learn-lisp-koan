...
; (defvar (one-counts) (two-counts) (three-counts) (four-counts) (five-counts) (six-counts))
(defun score (dice)
  ; You need to write this method
  (let* ((one-counts 0) (two-counts 0) (three-counts 0) (four-counts 0) (five-counts 0) (six-counts 0))
  (if (not dice)
      0
      (loop for x in dice
            do (
              cond
                ((= x 1) (incf one-counts))
                ((= x 2) (incf two-counts))
                ((= x 3) (incf three-counts))
                ((= x 4) (incf four-counts))
                ((= x 5) (incf five-counts))
                ((= x 6) (incf six-counts))
                )
            finally (
                cond
                ((> one-counts 2)
                  (return (+ 1000 (+ (* 100 (- one-counts 3)) (* 50 five-counts)))))
                ((> two-counts 2)
                    (return (+ 200  (+ (* 100 one-counts) (* 50 five-counts)))))
                ((> three-counts 2)
                    (return (+ 300  (+ (* 100 one-counts) (* 50 five-counts)))))
                ((> four-counts 2)
                    (return (+ 400  (+ (* 100 one-counts) (* 50 five-counts)))))
                ((> five-counts 2)
                  (return (+ 500  (+ (* 100 one-counts) (* 50 (- five-counts 3))))))
                ((> six-counts 2)
                  (return (+ 600  (+ (* 100 one-counts) (* 50 five-counts)))))
                (t
                  (return (+ (* 100 one-counts) (* 50 five-counts))))
                )))))
